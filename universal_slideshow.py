...
SHUFFLE_TIME = 5  # Default 5-second delay between images
        ...
        while True:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    last_update = time.time()
                    break
                elif event.key == pygame.K_s:  # Shuffle functionality from original script
                    random.shuffle(images)
                    index = 0  # Reset to first image after shuffle
                    last_update = time.time()
